package com.car_rental.Impl.ControllerImpl;

import com.car_rental.Interface.ControllerI.UserRidesControllerI;
import com.car_rental.Interface.ServiceI.UserRidesServiceI;
import com.car_rental.Logic.ControllerLogic.BaseController;
import com.car_rental.Logic.ModelLogic.Page;
import com.car_rental.Model.DatabaseModel.AuthUser;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.servlet.ModelAndView;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpSession;
import java.util.Optional;

@Controller
@RequestMapping("/customer")
public class UserRidesControllerImpl extends BaseController implements UserRidesControllerI {

    private UserRidesServiceI service;

    @Autowired
    public UserRidesControllerImpl(UserRidesServiceI service) {
        this.service = service;
    }

    @Override
    @GetMapping("/myrides")
    public ModelAndView Page(HttpServletRequest request, ModelMap modelMap, HttpSession session) {

       Optional<AuthUser> user = ExtractFromSessionAuthUser(session);

        if (user.isEmpty()) {
            return new ModelAndView("redirect:/login");
        }

        Optional<Page> page = this.service.GetPage(user.get().getId());

        if (page.isEmpty()) {
            return this.SendError("/customer", modelMap);
        }

        user.ifPresent(authUser -> page.get().setUser(authUser));
        page.get().Link(modelMap);

        return new ModelAndView("pages/user_rides", modelMap);
    }
}
